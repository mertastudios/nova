...
export default function Terms() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10 md:py-16 flex-grow w-full">
      <h1 className={`text-2xl md:text-3xl font-extrabold mb-6 md:mb-8 text-cyan-400 transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        {t(lang, 'terms.title')}
      </h1>

      <div className="space-y-6 text-slate-300 leading-relaxed text-sm">
        <p className={`text-slate-400 transition-all duration-1000 delay-100 ${visible ? 'opacity-100' : 'opacity-0'}`}>
          {t(lang, 'terms.updated')}
        </p>
